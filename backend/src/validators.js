...
module.exports = {
  ...
  register: wrap({
    body: {
      ...
      preferredCurrency: Joi.string().length(3).uppercase().default("USD"),
    },
  }),
  login: wrap({
    body: {
  ...
  resetPassword: wrap({
    body: { token: Joi.string().required(), password: password.required() },
  }),
  updateMe: wrap({
    body: {
  ...
  listAccounts: wrap({
    query: { includeArchived: Joi.string().valid("true", "false") },
  }),
  createAccount: wrap({
    body: {
      accountName: Joi.string().max(100).required(),
      accountType: Joi.string()
        .valid("cash", "bank", "credit", "savings", "investment", "custom")
        .required(),
  ...
  transfer: wrap({
    body: {
      sourceAccountId: id.required(),
      destinationAccountId: id.required(),
      amount: money.required(),
      date: Joi.date().iso().required(),
      description: Joi.string().max(200),
    },
  }),
  listTransactions: wrap({
    query: {
      ...pageQuery,
      accountId: id,
      categoryId: id,
  ...
  bulkDelete: wrap({
    body: { ids: Joi.array().items(id.required()).min(1).max(50).required() },
  }),
  recurringCreate: wrap({
    body: {
      accountId: id.required(),
  ...
  listCategories: wrap({
    query: { type: Joi.string().valid("income", "expense", "system") },
  }),
  report: wrap({
    body: {
      reportType: Joi.string().valid("monthly", "annual").required(),
      month: Joi.when("reportType", {
        is: "monthly",
  ...
  adminStatus: wrap({
    params: { id: id.required() },
    body: { status: Joi.string().valid("active", "suspended").required() },
  }),
  auditQuery: wrap({
    query: {
      ...pageQuery,
  ...
  categoryCreate: wrap({
    body: {
      ...
      nameAr: Joi.string().max(100).required(),
      type: Joi.string().valid("income", "expense", "system").required(),
      icon: Joi.string().max(50),
      color: hex,
  ...
  categoryUpdate: wrap({
    params: { id: id.required() },
    body: {
      nameEn: Joi.string().max(100),
      nameAr: Joi.string().max(100),
      type: Joi.string().valid("income", "expense", "system"),
      icon: Joi.string().max(50).allow(null),
      color: hex.allow(null),
